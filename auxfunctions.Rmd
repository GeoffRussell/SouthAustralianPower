...
df2022<-bind_rows(map2(l$s,l$t,getEnergySheet)) %>% left_join(pop2022,by=join_by(Country)) %>% 
...
c<-df2022 %>% inner_join(countries,by=join_by(Country)) %>% group_by(Country) %>% summarise(sum=sum(kWhPerCap)) %>% ungroup() %>% inner_join(df2022,by=join_by(Country))
